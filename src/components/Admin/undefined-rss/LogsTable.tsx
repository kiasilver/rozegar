  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Stats approximation based on current fetch or separate API? 
        // For accurate stats we usually need a separate endpoint, but let's calculate from current page or use total if available.
        // Actually, let's keep it simple for now or fetch stats separately if needed.
        // CarPriceLogs page calculates stats on frontend from loaded logs, which is wrong for pagination.
        // But here we can just show "Total" from pagination.

        // Let's count success/error from current batch for now to avoid complexity, or just show Total.
        // Or if API returns stats. API currently doesn't return full stats.
      }
    } catch (error: any) {

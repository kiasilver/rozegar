...
export default function RSSFeeds() {
  ...
  const fetchData = async () => {
    try {
      ...
      const sourcesContentType = sourcesRes.headers.get('content-type') || '';
      const categoriesContentType = categoriesRes.headers.get('content-type') || '';

      if (!sourcesContentType.includes('application/json')) {
    ...
    try {
      ...
      const updatePromises = sources.map(async (source) => {
        ...
        if (!contentType.includes('application/json')) {
          ...
          // If it's not JSON and not HTML, try to parse as JSON anyway
          try {
  ...
  const deleteSource = async (id: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json().catch(() => ({ error: 'خطا در حذف' }));

...
export const metadata: Metadata = {
  title: 'چند رسانه‌ای | سایت خبری',
  description: 'ویدیوها و اینفوگرافی‌های خبری',
  alternates: {
    canonical: '/media',
  },
};
...
export default async function MediaPage({ searchParams }: PageProps) {
  ...
  const activeTab = params.tab === 'infographic' ? 'infographic' : 'video';
  ...
  // Filter by video or infographic
  if (activeTab === 'video') {
    // فقط خبرهایی که ویدیو دارند
    ...
    // برای اینفوگرافی - همه خبرها با عکس (بدون نیاز به ویدیو)
    whereClause.image = {
      not: null,
    };
  }
          ...
          <div className="lg:col-span-8">
            ...
            <div className="mb-6 sm:mb-8">
              ...
              <p className="text-base sm:text-lg text-gray-600">
                {activeTab === 'video' ? 'ویدیوها' : 'اینفوگرافی‌ها'} ({totalPosts.toLocaleString('fa-IR')} مورد)
              </p>
            </div>
            ...
            {posts.length > 0 ? (
              ...
              <div className="text-center py-8 sm:py-12">
                <p className="text-base sm:text-lg text-gray-600">
                  {activeTab === 'video' ? 'ویدیویی یافت نشد.' : 'اینفوگرافی‌ای یافت نشد.'}
                </p>
              </div>
            )}
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-1 sm:gap-2 mt-6 sm:mt-8 flex-wrap">
                {currentPage > 1 && (
                  <Link
                    href={`/media?tab=${activeTab}&page=${currentPage - 1}`}
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => {
                  if (
                    ...
                    return (
                      <Link
                        key={pageNum}
                        href={`/media?tab=${activeTab}&page=${pageNum}`}

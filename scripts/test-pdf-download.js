 ...
 * استفاده از Node.js built-in fetch (بدون نیاز به dependency)
 */
...
async function testPDFDownload() {
  ...
  try {
    ...
    const newspapersWithPDF = (data.newspapers || []).filter((p) => p.pdfUrl);
    console.log(`📄 روزنامه‌های با PDF: ${newspapersWithPDF.length}\n`);

    // بررسی روزنامه‌های با PDF
    if (newspapersWithPDF.length > 0) {
      console.log('📋 لیست روزنامه‌های با PDF (5 مورد اول):');
      newspapersWithPDF.slice(0, 5).forEach((paper, index) => {
        console.log(`  ${index + 1}. ${paper.name || paper.persianName || 'نامشخص'}`);
        const pdfUrl = paper.pdfUrl || '';
        console.log(`     PDF URL: ${pdfUrl.substring(0, 80)}${pdfUrl.length > 80 ? '...' : ''}`);
      });
    ...
    if (error.name === 'AbortError' || error.message?.includes('timeout') || error.code === 'ETIMEDOUT') {
      ...
      console.error(`💡 بررسی کنید که سرور روی ${BASE_URL} در حال اجرا باشد`);
